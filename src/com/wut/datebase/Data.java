...
public class Data {
	...
	public ReturnUser validateregister(User user){
		...
		String sql = "insert into Sdnote_User(SU_userName,SU_password,SU_Token,SU_Email,SU_phone,SU_age,SU_sex) "
				+ "values ('"+username+"','"+password+"','"+uid+"','"+Email+"','"+phone+"','"+age+"','"+sex+"')";
...
//	public ArrayList<Integer> phone = new Arraylist<integer>();
	public void init() throws SQLException{
		...
		String sql = "select  SU_userID,SU_userName,SU_Token,SU_Email,SU_phone from Sdnote_User";
	...
	public LoginUser validateUser(String username , String password ) throws SQLException{
		...
		String sql = "select SU_userID,SU_userName,SU_password,SU_Token from Sdnote_User where SU_userName = "+"\'"+username+"\'";
		
		ResultSet resultSetes = stst.executeQuery(sql);
		while(resultSetes.next()){
			if(resultSetes.getString("SU_password").equals(password)){
				
				return new LoginUser("200",resultSetes.getString("SU_userName"),"",resultSetes.getString("SU_userID"),resultSetes.getString("SU_Token"));
			}
			//401:密码错误
	...
	//用户增加标签
	 public StateInformation addNote(Note note){
		...
		boolean T = false;
		for(int i =0 ;i<userValidate.size() ; i++){
			if(userValidate.get(i).get("SU_userID").equals(UserID)&&userValidate.get(i).get("SU_Token").equals(Token)){
				T = true;
			}
		}
		if (T) {
			String sql = "insert into Sdnote_Note(SN_userID,SN_Token,SN_title,SN_time,SN_content,SN_tag) "
					+ "values ('"+UserID+"','"+Token+"','"+Title+"','"+time+"','"+Content+"','"+Tag+"')";
			try {
				//成功
				stst.executeUpdate(sql);
	...
	//用户更新标签
		 public StateInformation UpdateNote(Note note){
			...
			if (T) {
				String sql = "UPDATE Sdnote_Note SET  SN_title = '"+Title+"',SN_time = '"+NoteTime+"',SN_content = '"+Content+"',SN_tag='"+Tag+"' WHERE SN_noteID = '"+SN_noteID+"'";
		 ...
		 public Map<String, Object> getAllNote(JSONObject note){
			...
			if (T) {
				...
				try {
					...
					ResultSet resultSetes = stst.executeQuery(sql);
					while (resultSetes.next()) {
						a.add(new returnNote(resultSetes.getString("SN_noteID"),resultSetes.getString("SN_title"),resultSetes.getString("SN_time"),resultSetes.getString("SN_content"),resultSetes.getString("SN_tag")));
					}
		 ...
		 public StateInformation addRemind(Remind remind){
			...
			if (T) {
				String sql = "insert into Sdnote_Remind(SR_userID,SR_Token,SR_title,SR_time,SR_stratTime,SR_endTime,SR_remindTime,SR_content,SR_tag) "
						+ "values ('"+UserID+"','"+Token+"','"+Title+"','"+Time+"','"+stratTime+"','"+endTime+"','"+remindTime+"','"+Content+"','"+Tag+"')";
				try {
					//成功
					stst.executeUpdate(sql);
		 ...
		 public StateInformation UpdateRemind(Remind remind){
			...
			if (T) {
				String sql = "UPDATE Sdnote_Remind SET  SR_title = '"+Title+"',SR_time = '"+Time+"',SR_stratTime ="
						+ "'"+stratTime+"',SR_endTime = '"+endTime+"',SR_remindTime = '"+remindTime+"',SR_content = "
								+ "'"+Content+"',SR_tag='"+Tag+"' WHERE SR_remindID = '"+remindID+"'";
						System.out.println(sql);
				try {
					//成功
					int i = stst.executeUpdate(sql);
				...
				if (T) {
					...
					try {
						...
						while (resultSetes.next()) {
							...
							a.add(new returnRemind(resultSetes.getString("SR_remindID"),resultSetes.getString("SR_title"),resultSetes.getString("SR_time"),
									resultSetes.getString("SR_stratTime"),resultSetes.getString("SR_endTime"),resultSetes.getString("SR_remindTime"),resultSetes.getString("SR_content"),resultSetes.getString("SR_tag")));
